...
if __name__ == "__main__":
    ...
    title_features_fake = title_features_fake[:min(title_features_fake.type.count(),75)]#,title_features_reliable.type.count())]
    title_features_reliable = title_features_reliable[:min(title_features_fake.type.count(),75)]#,title_features_reliable.type.count())]
    ...
    body_features_fake = body_features_fake[:min(body_features_fake.type.count(),75)]#],body_features_reliable.type.count())]
    body_features_reliable = body_features_reliable[:min(body_features_fake.type.count(),75)]#,body_features_reliable.type.count())]
    ...
    full_body_features = body_features# = body_features.drop(columns=ignore_features)
    full_title_features = title_features# = title_features.drop(columns=ignore_features)
    ...
    #body_features = full_body_features[['type', 'NN', 'TTR', 'WC', 'quote']]
    #print(body_features)
    ...
    folds = 5
    cv = sklearn.model_selection.KFold(n_splits=folds, random_state=42, shuffle=True)
    #title_features = full_title_features[['type', 'per_stop', 'NN', 'avg_wlen', 'FK']]
